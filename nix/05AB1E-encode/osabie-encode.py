...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="osabie-encode",
        description="Encode from utf-8 to osabie encoding")
    parser.add_argument(
        "-d", "--decode", help="Decode from osabie encoding to utf-8 instead", action="store_true")
    parser.add_argument(
        "INPUT_FILE", help="input file", action="store", type=str)
    parser.add_argument(
        "OUTPUT_FILE", help="output file, standard output if omitted", action="store", type=str, nargs='?')
    ...
    if not DECODE:
        ...
        osabie = utf8_to_osabie(infile.read()).encode(encoding="charmap")
        ...
        utf8 = osabie_to_utf8(infile.read())
